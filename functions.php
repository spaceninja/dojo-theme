...
if ( function_exists('register_sidebar') ) {
	...
	register_sidebar( array(
		...
		'after_widget' => '</section><!-- end widget -->',
		'before_title' => '<h4>',
		'after_title' => '</h4>',
	));
	register_sidebar( array(
 ...
 * Kill the feed links so we can use Atom instead of RSS2
 * Kill the "generated by" line because it's a security risk
 * ref: http://wpengineer.com/1438/wordpress-header/
 */
	...
	echo $before_title . "Subscribe" . $after_title . "\n"; ?>
		...
		<ul class="rss">
			...
			if (is_single()) {
				...
				print '<li><a href="' . get_post_comments_feed_link( $wp_query->queried_object->ID, 'atom' ) . '">Comments on ' . $wp_query->queried_object->post_title . '</a></li>';

			} elseif (is_category()) {
				// add category feed on category archives
				print '<li><a href="' . get_category_feed_link( $wp_query->queried_object->term_id, 'atom' ) . '">Posts in the ' . $wp_query->queried_object->name . ' category</a></li>';

			} elseif (is_tag()) {
				// add tag feed on tag archives
				print '<li><a href="' . get_tag_feed_link( $wp_query->queried_object->term_id, 'atom' ) . '">Posts tagged with ' . $wp_query->queried_object->name . '</a></li>';

			} elseif (is_author()) {
				// add author feed on author pages
				print '<li><a href="' . get_author_feed_link( $wp_query->queried_object->ID, 'atom' ) . '">Posts by ' . $wp_query->queried_object->display_name . '</a></li>';
